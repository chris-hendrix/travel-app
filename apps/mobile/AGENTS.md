...
- **A store's data comes from an injected source, never a module-level mock import.** The seam lives in `lib/sources.ts`: the provider takes a `source` prop defaulting to the mock, and the wiring swaps in an API source without touching the screens. Actions and data travel on separate contexts, so a screen that only writes never re-renders when data changes anywhere: the actions value is created once and keeps its identity across writes, the data value is cached between writes and changes on a real one.
