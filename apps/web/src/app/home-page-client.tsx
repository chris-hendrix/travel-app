...
import { Calendar, Building2, PartyPopper, Plane, Loader2 } from "lucide-react";
...
export function HomePageClient() {
  ...
  // Redirect authenticated web users to /trips.
  // Native redirect is handled by the inline <script> in page.tsx
  // (runs before React hydration to avoid a flash of landing page content).
  useEffect(() => {
  ...
  // Show loading spinner while auth state is resolving (web only — native
  // is handled by the pre-React <script> redirect before this mounts).
  if (!isNative() && loading) {
    return (
  ...
  // Don't render the landing page while redirecting an authenticated user
  if (!isNative() && user) {
    return null;
  }
