...
- **Placeholders come from one file, and backend gaps are ledger-tracked.** All event/stay images come from `lib/placeholder.ts` (`placeholderPhoto`); a screen that needs a backend column or field it does not have carries a ledger-tracked comment naming it. Those comments are ledger-tracked (rows #1–#6 in the wiring plan) — no other backend-gap comments may exist.
...
E2E runs take file paths, not `--grep`: the flag is swallowed through test-exec's bash -c, so pass the spec path (`pnpm test:e2e tests/e2e/auth-journey.spec.ts`). The Expo web build serves on `http://localhost:8081`, which must be present in the API's `FRONTEND_URL` or the browser's CORS preflight fails.
