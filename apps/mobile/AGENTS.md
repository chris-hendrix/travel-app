...
- **Placeholders come from one file, and backend gaps are ledger-tracked.** All event/stay images come from `lib/placeholder.ts` (`placeholderPhoto`); a screen that needs a backend column or field it does not have carries a `// TODO(BE):` naming it. Those comments are ledger-tracked (rows #1–#6 in the wiring plan) — no other `TODO(BE)` may exist.
